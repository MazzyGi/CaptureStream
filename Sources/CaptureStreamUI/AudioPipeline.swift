...
public final class AudioPipeline: NSObject, AVCaptureAudioDataOutputSampleBufferDelegate {
    ...
    /// 输入增益（0~1），attach 前设置。
    public var inputVolume: Float = 1.0
    ...
    private func firstAudioCaptureDevice() -> AVCaptureDevice? {
        if #available(macOS 14.0, *) {
            return AVCaptureDevice.DiscoverySession(deviceTypes: [.external, .builtInMicrophone],
                                                    mediaType: .audio, position: .unspecified).devices.first
        }
        return AVCaptureDevice.DiscoverySession(deviceTypes: [.externalUnknown],
                                                mediaType: .audio, position: .unspecified).devices.first
    }
    ...
    private func pushSamples(_ interleaved: [Float]) {
        // 数字输入增益（macOS AVCaptureDevice 无硬件音量 API）
        ...
        if gain == 1.0 {
            ...
            for s in interleaved {
                ring[ringWrite] = max(-1, min(1, s * gain))
                ringWrite = (ringWrite + 1) % ringCapacity
            }
        }
    }
