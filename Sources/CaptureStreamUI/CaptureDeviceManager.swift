...
public final class CaptureDeviceManager: ObservableObject {
    ...
    public func refresh() {
        ...
        for d in AVCaptureDevice.DiscoverySession(
            deviceTypes: [.externalUnknown], mediaType: .audio, position: .unspecified).devices {
    ...
    public static func pixelFormatName(_ codec: FourCharCode) -> String {
        ...
        default:
            let c = codec.bigEndianBytes
            return String(bytes: [c.0, c.1, c.2, c.3], encoding: .ascii) ?? "0x\(String(codec, radix: 16))"
        }
    }
