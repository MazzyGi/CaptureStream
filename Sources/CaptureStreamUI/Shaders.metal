...
struct YUVParams {
    ...
    float sharpen;   // 0 = 关闭；>0 unsharp 强度
};
...
// ---------- 片元：Nearest / Bilinear（采样器区分）+ 可选锐化 ----------

static inline float4 applySharpen(float4 c, float4 blur, float amount) {
    if (amount <= 0.0) return c;
    return clamp(c + (c - blur) * amount, 0.0, 1.0);
}
...
fragment float4 fragmentSample(BlitVertexOut in [[stage_in]],
    ...
    if (p.isRGB != 0) {
        float4 c = srcY.sample(smp, in.uv);
        if (p.sharpen > 0.0) {
            uint2 dims = uint2(srcY.get_width(), srcY.get_height());
            float2 px = float2(1.0) / float2(dims);
            float4 blur = (srcY.sample(smp, in.uv + float2(px.x, 0)) +
                           srcY.sample(smp, in.uv - float2(px.x, 0)) +
                           srcY.sample(smp, in.uv + float2(0, px.y)) +
                           srcY.sample(smp, in.uv - float2(0, px.y))) * 0.25;
            c = applySharpen(c, blur, p.sharpen);
        }
        return c;
    }
    float y = srcY.sample(smp, in.uv).r;
    float2 uv = sampleChroma(srcC, srcV, smp, in.uv, p);
    float3 rgb = yuvToRGB(y, uv, p.fullRange != 0);
    float4 c = float4(saturate(rgb), 1.0);
    if (p.sharpen > 0.0) {
        // 锐化作用于亮度域（Y），避免色度噪点放大
        uint2 dims = uint2(srcY.get_width(), srcY.get_height());
        float2 px = float2(1.0) / float2(dims);
        float yBlur = (srcY.sample(smp, in.uv + float2(px.x, 0)).r +
                       srcY.sample(smp, in.uv - float2(px.x, 0)).r +
                       srcY.sample(smp, in.uv + float2(0, px.y)).r +
                       srcY.sample(smp, in.uv - float2(0, px.y)).r) * 0.25;
        float ySharp = clamp(y + (y - yBlur) * p.sharpen, 0.0, 1.0);
        float3 rgb2 = yuvToRGB(ySharp, uv, p.fullRange != 0);
        c = float4(saturate(rgb2), 1.0);
    }
    return c;
}
...
fragment float4 fragmentBlend(BlitVertexOut in [[stage_in]],
                              texture2d<float> lumaA [[texture(0)]],
                              texture2d<float> lumaB [[texture(1)]],
                              texture2d<float> chromaA [[texture(2)]],
                              texture2d<float> chromaB [[texture(3)]],
                              sampler smp [[sampler(0)]],
                              constant BlendParams &p [[buffer(0)]]) {
    if (p.isNV12 == 0) {
        float4 a = lumaA.sample(smp, in.uv);
        float4 b = lumaB.sample(smp, in.uv);
        return mix(a, b, p.t);
    }
    float ya = lumaA.sample(smp, in.uv).r;
    float yb = lumaB.sample(smp, in.uv).r;
    float2 ca = chromaA.sample(smp, in.uv).rg;
    float2 cb = chromaB.sample(smp, in.uv).rg;
    float y = mix(ya, yb, p.t);
    float2 c = mix(ca, cb, p.t);
    YUVParams yp;
    yp.fullRange = p.fullRange;
    yp.planar3 = 0;
    yp.isRGB = 0;
    yp.sharpen = 0;
    return float4(saturate(yuvToRGB(y, c, p.fullRange != 0)), 1.0);
}
