...
public struct MainWindow: View {
    ...
    private var toolbar: some View {
        HStack(spacing: 12) {
            ...
            Picker("帧率", selection: $appState.selectedFPS) {
                Text("默认").tag(Int?.none)
                ForEach(appState.availableFPS, id: \.self) { fps in
                    Text("\(fps) FPS").tag(Int?.some(fps))
                }
            }
            .frame(minWidth: 110)
            .disabled(appState.availableFPS.isEmpty)
